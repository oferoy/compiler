...
public class AstDecClass extends AstDec {
    ...
    public Type semantMe()
    {
        ...
        // Grammar prepends fields, so list is (lastDeclared, ..., firstDeclared). Reverse to get declaration order so first field gets offset 4.
        java.util.List<AstTypeName> fieldOrder = new java.util.ArrayList<>();
        for (AstTypeNameList it = fields; it != null; it = it.tail)
            fieldOrder.add(it.head);
        java.util.Collections.reverse(fieldOrder);

        TypeClassMember memberList = null;
        TypeClassMember tail       = null;

        for (AstTypeName field : fieldOrder) {
